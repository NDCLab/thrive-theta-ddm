...
for s = 1:length(erpDat_subIds)
    ...
    set(gcf, 'Position', [0 0 1440 900]);
    grid on;
    saveas(gcf, strcat(erpDat_subIds{s},'.png'));
...
%create variable names for count trials output and write to disk
outputHeader = {['id, ERN_soc, CRN_soc, ERN_nonsoc, CRN_nonsoc, ERN_min_CRN_diff_soc, ERN_min_CRN_diff_nonsoc,' ...
    ' PE_error_soc, PE_corr_soc, PE_error_nonsoc,  PE_corr_nonsoc, PE_err_min_corr_diff_soc, PE_err_min_corr_diff_nonsoc ']};
...
output(:,1) = str2double(erpDat_subIds);
...
for comp = 1:length(clustCell) %loop through component clusters
    ...
    %pull out conditions of interest for all subs, and average over time
    %range of interest
    s_resp_incon_error_avgTime = mean(newData(:,1,:,compRange),4);
    s_resp_incon_corr_avgTime = mean(newData(:,2,:,compRange),4);
    ns_resp_incon_error_avgTime = mean(newData(:,3,:,compRange),4);
    ns_resp_incon_corr_avgTime = mean(newData(:,4,:,compRange),4);
    ...
    s_resp_incon_error_avgTimeClust = mean(s_resp_incon_error_avgTime(:,:,cluster),3);
    s_resp_incon_corr_avgTimeClust = mean(s_resp_incon_corr_avgTime(:,:,cluster),3);
    ns_resp_incon_error_avgTimeClust = mean(ns_resp_incon_error_avgTime(:,:,cluster),3);
    ns_resp_incon_corr_avgTimeClust = mean(ns_resp_incon_corr_avgTime(:,:,cluster),3);
    ...
    output(:,i) = s_resp_incon_error_avgTimeClust;
    output(:,i+1) = s_resp_incon_corr_avgTimeClust;
    output(:,i+2) = ns_resp_incon_error_avgTimeClust;
    output(:,i+3) = ns_resp_incon_corr_avgTimeClust;
    output(:,i+4) = s_resp_incon_error_avgTimeClust_diff;
    output(:,i+5) = ns_resp_incon_error_avgTimeClust_diff;
    i= i+6;
...
dlmwrite(strcat('thrive_compMeans_', date, '.csv'), output, 'delimiter', ',', '-append', 'precision',10);