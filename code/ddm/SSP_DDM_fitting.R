...
data_dir = sprintf("%sderivatives/behavior/%s/", analysis_path, session)
...
importDat = read.csv(latest_file, header = TRUE) # data has header
...
for (cond in 1:2) {
  ...
  for (s in 1:length(subList)) {
    ...
    # loop through and calculate separate fits for the previous accuracy conditions
    #0: postError
    #1: postCorr
